/**
 * search_eu_implementations — Search EU directives/regulations that have Seychelles implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_seychelles_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  seychelles_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as seychelles_statute_count
  ...
  if (input.has_seychelles_implementation) {
    sql += ' HAVING seychelles_statute_count > 0';
  }
