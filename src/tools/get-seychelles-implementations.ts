/**
 * get_seychelles_implementations — Find Seychelles statutes implementing a specific EU directive/regulation
...
export interface GetSeychellesImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface SeychellesImplementationResult {
  document_id: string;
...
export async function getSeychellesImplementations(
  db: InstanceType<typeof Database>,
  input: GetSeychellesImplementationsInput,
): Promise<ToolResponse<SeychellesImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SeychellesImplementationResult[];
