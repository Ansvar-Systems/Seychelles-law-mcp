#!/usr/bin/env tsx
/**
 * Seychelles Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawseychelles.com. Seychelles proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawseychelles.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'seychelles-law-mcp/1.0 (https://github.com/Ansvar-Systems/seychelles-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Seychelles Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawseychelles.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawseychelles.com (Federal Negarit Gazette PDFs)`);
