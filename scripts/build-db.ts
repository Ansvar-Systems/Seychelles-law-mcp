#!/usr/bin/env tsx
/**
 * Database builder for Seychelles Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Seychelles Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Seychelles statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'lawseychelles.com');
