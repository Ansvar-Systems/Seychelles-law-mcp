#!/usr/bin/env tsx
/**
 * Drift detection for Seychelles Law MCP.
 *
 * Checks if upstream Seychelles Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Seychelles Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'seychelles-law-mcp/1.0 drift-detect' },
      });
