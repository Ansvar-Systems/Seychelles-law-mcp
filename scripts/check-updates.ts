#!/usr/bin/env tsx
/**
 * Seychelles Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawseychelles.com';
const PORTAL_NAME = 'Seychelles Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/seychelles-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Seychelles Law MCP — Data Freshness Check');
